...
export function replaceMergeTags(
  ...
  // RE851D multi-property support: tags ending in _1.._5 route to the matching
  // CSR Property record (property1.*, property2.*, ...). When the index is out
  // of range (no propertyN.* in fieldValues), the tag resolves to blank so the
  // unused PROPERTY block prints empty. Pure additive — non-suffixed tags fall
  // through unchanged.
  const PR_INDEX_SUFFIX_MAX = 5;
  // Map base merge-tag identifiers to the bridged propertyN.<suffix> key.
  // Mirrors prKeyToSuffix in generate-document/index.ts plus extras the
  // RE851D template needs for PART 2 / PROPERTY #N blocks.
  const prBaseToPropertySuffix: Record<string, string> = {
    pr_p_street: "street",
    pr_p_city: "city",
    pr_p_state: "state",
    pr_p_zip: "zip",
    ...
    pr_p_totalSenior: "total_senior_encumbrance",
    pr_p_totalSeniorPlusLoan: "total_senior_plus_loan",
    ln_p_loanToValueRatio: "loan_to_value_ratio",
  };

  const resolveIndexedPropertyTag = (rawTag: string): { resolvedTag: string; isPropIndexed: boolean; idx: number } => {
    const m = rawTag.match(/^(.+?)_([1-9]\d*)$/);
    if (!m) return { resolvedTag: rawTag, isPropIndexed: false, idx: 0 };
    const base = m[1];
    const idx = parseInt(m[2], 10);
    if (idx < 1 || idx > PR_INDEX_SUFFIX_MAX) return { resolvedTag: rawTag, isPropIndexed: false, idx: 0 };
    const suffix = prBaseToPropertySuffix[base];
    if (!suffix) return { resolvedTag: rawTag, isPropIndexed: false, idx: 0 };
    return { resolvedTag: `property${idx}.${suffix}`, isPropIndexed: true, idx };
  };

  // Detect which property indices have any data so blocks for missing
  // properties render blank instead of falling back to property1.
  const propertyIndicesWithData = new Set<number>();
  for (const [k] of fieldValues.entries()) {
    const pm = k.match(/^property(\d+)\./i);
    if (pm) propertyIndicesWithData.add(parseInt(pm[1], 10));
  }

  for (const tag of tags) {
    // Detect index-suffixed property tag (_1.._5) and route to propertyN.* key.
    const indexed = resolveIndexedPropertyTag(tag.tagName);
    if (indexed.isPropIndexed) {
      // Property record at this index does not exist → blank the tag.
      if (!propertyIndicesWithData.has(indexed.idx)) {
        tagReplacementMap.set(tag.fullMatch, "");
        continue;
      }
      const resolvedIndexed = getFieldData(indexed.resolvedTag, fieldValues);
      let resolvedValue = "";
      if (resolvedIndexed?.data) {
        const transform = tag.inlineTransform || fieldTransforms.get(indexed.resolvedTag);
        if (transform) {
          resolvedValue = applyTransform(resolvedIndexed.data.rawValue, transform);
        } else {
          resolvedValue = formatByDataType(resolvedIndexed.data.rawValue, resolvedIndexed.data.dataType);
        }
        if (resolvedIndexed.data.dataType === 'currency' && resolvedValue.startsWith('$')) {
          resolvedValue = resolvedValue.substring(1);
        }
        replacedFieldKeys.add(indexed.resolvedTag);
      }
      tagReplacementMap.set(tag.fullMatch, escapeXmlValue(resolvedValue));
      continue;
    }
