...
interface ChargeRow {
  id: string; date: string; description: string; amount: string; status: string; reference: string;
}

const COLUMNS = [
  { id: 'date', label: 'Date' }, { id: 'description', label: 'Description' },
  { id: 'amount', label: 'Amount' }, { id: 'status', label: 'Status' },
  { id: 'reference', label: 'Reference' },
];

const LenderCharges: React.FC<{ lenderId: string }> = () => {
    ...
    <div className="space-y-4">
      ...
      <div className="border border-border rounded-lg overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow><TableCell colSpan={COLUMNS.length} className="text-center py-8 text-muted-foreground">No charge records found.</TableCell></TableRow>
            ) : filtered.map(r => (
