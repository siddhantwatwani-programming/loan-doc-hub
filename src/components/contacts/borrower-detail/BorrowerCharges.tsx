...
import { Search, Plus, Download, Settings2 } from 'lucide-react';
...
interface ChargeRow {
  id: string; loanReference: string; chargeType: string; amount: string; status: string; date: string; description: string;
}
...
const EMPTY_CHARGE: Omit<ChargeRow, 'id'> = { loanReference: '', chargeType: '', amount: '', status: '', date: '', description: '' };
...
const BorrowerCharges: React.FC<Props> = ({ contactDbId }) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data?.contact_data) { const cd = data.contact_data as Record<string, any>; if (Array.isArray(cd._charges)) setRows(cd._charges); }
    };
    if (contactDbId) load();
  }, [contactDbId]);

  const persist = useCallback(async (updated: ChargeRow[]) => {
    setIsSaving(true);
    try {
      ...
      const merged = { ...existing, _charges: updated } as any;
    ...
    } catch { toast.error('Failed to save charges'); } finally { setIsSaving(false); }
  }, [contactDbId]);

  const handleAdd = useCallback(async () => {
    const row: ChargeRow = { ...newCharge, id: crypto.randomUUID() };
    const updated = [...rows, row];
    await persist(updated);
    setRows(updated); setNewCharge(EMPTY_CHARGE); setAddOpen(false);
    toast.success('Charge added');
    logContactEvent(contactDbId, 'Charges', [{ fieldLabel: 'Charge Added', oldValue: '', newValue: row.description || 'New charge' }]);
  }, [newCharge, rows, persist, contactDbId]);

  const handleDeleteSelected = useCallback(async () => {
    if (selectedRows.size === 0) return;
    const updated = rows.filter(r => !selectedRows.has(r.id));
    await persist(updated); setRows(updated); setSelectedRows(new Set());
  ...
  const filtered = useMemo(() => { let r = rows; if (search) { const q = search.toLowerCase(); r = r.filter(row => Object.values(row).some(v => String(v).toLowerCase().includes(q))); } if (sortCol && sortDir) { r = [...r].sort((a, b) => { const av = (a as any)[sortCol] || ''; const bv = (b as any)[sortCol] || ''; return sortDir === 'asc' ? av.localeCompare(bv) : bv.localeCompare(av); }); } return r; }, [rows, search, sortCol, sortDir]);
  ...
  const handleExport = () => {
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'borrower_charges.csv'; a.click(); URL.revokeObjectURL(url);
  };
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Popover>
            ...
            <PopoverContent className="w-56 p-3" align="end">
              <div className="space-y-2"><span className="text-sm font-medium">Toggle Columns</span>
                {ALL_COLUMNS.map(c => (<div key={c.id} className="flex items-center gap-2"><Checkbox id={`bc-col-${c.id}`} checked={visibleColumns.has(c.id)} onCheckedChange={() => toggleColumn(c.id)} /><label htmlFor={`bc-col-${c.id}`} className="text-xs cursor-pointer">{c.label}</label></div>))}
              </div>
            </PopoverContent>
          </Popover>
          ...
          <Button size="sm" variant="outline" className="gap-1 h-8 text-xs" onClick={() => setAddOpen(true)}><Plus className="h-3.5 w-3.5" /> Add Charge</Button>
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow><TableCell colSpan={activeColumns.length + 1} className="text-center py-8 text-muted-foreground text-sm">No charge records found.</TableCell></TableRow>
            ) : filtered.map(r => (
              <TableRow key={r.id} className={selectedRows.has(r.id) ? 'bg-primary/5' : ''}>
                ...
                {activeColumns.map(c => <TableCell key={c.id} className="whitespace-nowrap text-xs">{c.id === 'amount' && (r as any)[c.id] ? `$ ${(r as any)[c.id]}` : (r as any)[c.id] || '-'}</TableCell>)}
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-3">
            {ALL_COLUMNS.map(col => (<div key={col.id} className="space-y-1"><Label className="text-xs">{col.label}</Label><Input className="h-8 text-xs" value={(newCharge as any)[col.id] || ''} onChange={e => setNewCharge(prev => ({ ...prev, [col.id]: e.target.value }))} placeholder={col.label} /></div>))}
          </div>
          <DialogFooter><Button variant="outline" size="sm" onClick={() => setAddOpen(false)}>Cancel</Button><Button size="sm" onClick={handleAdd} disabled={isSaving}>{isSaving ? 'Saving...' : 'Add Charge'}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
