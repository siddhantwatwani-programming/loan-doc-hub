...
type SubSection = 'lender' | 'authorized_party' | 'banking' | 'tax_info';
...
export const ContactLenderDetail: React.FC<Props> = ({ contact, onBack, onUpdated }) => {
  const [activeSection, setActiveSection] = useState<SubSection>('lender');
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const fullName = values['lender.full_name'] || '';
      const firstName = values['lender.first_name'] || '';
      const lastName = values['lender.last_name'] || '';
      const email = values['lender.email'] || '';
      const phone = values['lender.phone.cell'] || values['lender.phone.work'] || '';
      const city = values['lender.primary_address.city'] || '';
      const state = values['lender.primary_address.state'] || '';
      ...
      if (updated) {
        onUpdated(updated);
        toast({ title: 'Contact saved', description: 'Lender contact updated successfully.' });
      }
    } catch (err) {
  ...
  const renderContent = () => {
    const commonProps = { fields: [], values, onValueChange: handleValueChange, disabled: false, calculationResults: {} };
    switch (activeSection) {
      case 'lender': return <LenderInfoForm {...commonProps} />;
      case 'authorized_party': return <LenderAuthorizedPartyForm {...commonProps} />;
      case 'banking': return <LenderBankingForm {...commonProps} />;
      case 'tax_info': return <LenderTaxInfoForm {...commonProps} />;
      default: return null;
    }
  };
