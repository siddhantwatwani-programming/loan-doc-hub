...
import { Upload, Download, Trash2, MoreVertical, Mail, FileText, RefreshCw, Settings } from 'lucide-react';
...
interface PropertyAttachmentFormProps {
...
export const PropertyAttachmentForm: React.FC<PropertyAttachmentFormProps> = ({
  values,
  onValueChange,
  disabled = false,
  propertyId,
  ...
  // Parse attachments from values (stored as JSON in property1.attachments)
  const getAttachments = useCallback((): Attachment[] => {
    const attachmentsJson = values['property1.attachments'];
  ...
  const handleUpload = useCallback(() => {
    ...
    input.onchange = (e) => {
      ...
      if (files && files.length > 0) {
        ...
        onValueChange('property1.attachments', JSON.stringify(newAttachments));
  ...
  const handleDelete = useCallback(() => {
    ...
    onValueChange('property1.attachments', JSON.stringify(updatedAttachments));
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-lg text-primary">Property Attachments</h3>
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            size="icon"
            className="h-8 w-8"
            onClick={() => toast.info('View document functionality')}
            disabled={disabled || selectedAttachments.length === 0}
            title="View Document"
          >
        ...
        <Table>
          ...
          <TableBody>
            {attachments.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-8 text-muted-foreground bg-muted/30">
...
export default PropertyAttachmentForm;
